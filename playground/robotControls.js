...
function showAlert(type, message, duration = 3000) {
  const alertId = type === 'servo_limit' ? 'jointLimitAlert' : 'servoLimitAlert';
...
/**
 * 获取排序后的关节名称列表 (servo1, servo2, ...)
 * @param {Object} robot 
 * @returns {string[]}
 */
function getSortedJointNames(robot) {
...
export function setupKeyboardControls(robot) {
  ...
  // 默认的按键-舵机映射
  const keyMappings = {
    '1': { jointIndex: 0, direction: -1 }, // 1号舵机 减
    'q': { jointIndex: 0, direction: 1 },  // 1号舵机 加
    '2': { jointIndex: 1, direction: -1 }, // 2号舵机 减
    'w': { jointIndex: 1, direction: 1 },  // 2号舵机 加
    '3': { jointIndex: 2, direction: -1 }, // 3号舵机 减
    'e': { jointIndex: 2, direction: 1 },  // 3号舵机 加
    '4': { jointIndex: 3, direction: -1 }, // 4号舵机 减
    'r': { jointIndex: 3, direction: 1 },  // 4号舵机 加
    '5': { jointIndex: 4, direction: -1 }, // 5号舵机 减
    't': { jointIndex: 4, direction: 1 },  // 5号舵机 加
    '6': { jointIndex: 5, direction: -1 }, // 6号舵机 减
    'y': { jointIndex: 5, direction: 1 },  // 6号舵机 加
  };
  
  // 获取机器人实际的舵机(关节)名称并排序
  const jointNames = getSortedJointNames(robot);
  console.log('Available servos (sorted):', jointNames);
  ...
  function updateJoints() {
        ...
        if (jointIndex < jointNames.length) {
          ...
          if (robot.joints[jointName]) {
            ...
            if (!isJointWithinLimits(robot.joints[jointName], newValue)) {
              console.warn(`Servo ${jointName} would exceed its limits. Movement prevented.`);
              // 显示虚拟舵机限位提醒
              showAlert('servo_limit', `Servo ${jointName} has reached its limit!`);
              return; // 跳过这个舵机的更新
            }
    ...
    // 同步更新角度直控面板的数值
    updateAngleControlUI();
  }

  /**
   * 更新角度直控面板的UI数值
   */
  function updateAngleControlUI() {
    if (!robot || !robot.joints) return;
    
    // 如果用户正在输入，跳过自动更新，防止覆盖用户的输入
    if (document.activeElement.classList.contains('angle-input')) {
      return;
    }
    ...
    jointNames.forEach((jointName, index) => {
      ...
      if (joint) {
        const angleDeg = Math.round(MathUtils.radToDeg(joint.angle));
        
        const slider = document.querySelector(`.angle-slider[data-servo="${servoId}"]`);
        const input = document.querySelector(`.angle-input[data-servo="${servoId}"]`);
        
        if (slider && parseInt(slider.value) !== angleDeg) {
          slider.value = angleDeg;
        }
        if (input && parseInt(input.value) !== angleDeg) {
 ...
 */
export function setupAngleDirectControl(robot) {
  if (!robot || !robot.joints) return;

  const jointNames = getSortedJointNames(robot);

  // 为所有滑动条和输入框添加事件监听
  const sliders = document.querySelectorAll('.angle-slider');
  const inputs = document.querySelectorAll('.angle-input');

  const handleControlChange = (e) => {
    const servoId = parseInt(e.target.getAttribute('data-servo'));
    const jointIndex = servoId - 1;
    const valueDeg = parseFloat(e.target.value);
    const valueRad = MathUtils.degToRad(valueDeg);
    ...
    if (jointIndex < jointNames.length) {
      ...
      if (joint) {
        ...
        const slider = document.querySelector(`.angle-slider[data-servo="${servoId}"]`);
        const input = document.querySelector(`.angle-input[data-servo="${servoId}"]`);
        if (slider) slider.value = valueDeg;
        if (input) input.value = valueDeg;

        // 检查限制
        if (!isJointWithinLimits(joint, valueRad)) {
          console.warn(`Servo ${jointName} would exceed its limits. Movement prevented.`);
          showAlert('servo_limit', `Servo ${jointName} has reached its limit!`);
          return;
        }
        ...
        if (isConnectedToRealRobot) {
          ...
          // 4096 对应 2π
          // 步数 = 2048 + (弧度 * 4096 / 2π)
          const targetPosition = Math.round(2048 + (valueRad * 4096 / (2 * Math.PI)));
          
          servoCommStatus[servoId].status = 'pending';
          updateServoStatusUI();

          writeServoPosition(servoId, targetPosition)
            .then(success => {
              if (success) {
                joint.setJointValue(valueRad);
                servoCurrentPositions[servoId] = targetPosition;
                servoLastSafePositions[servoId] = targetPosition;
                servoCommStatus[servoId].status = 'success';
              } else {
                showAlert('servo', `Servo ${servoId} direct move failed!`);
              }
              updateServoStatusUI();
            })
            .catch(error => {
              console.error(`Error in direct move for servo ${servoId}:`, error);
              servoCommStatus[servoId].status = 'error';
              updateServoStatusUI();
            });
        } else {
  ...
  sliders.forEach(slider => {
    slider.addEventListener('input', handleControlChange);
  });
 ...
 * @param {Object} robot - 机器人对象
 */
export function setupWebSocketControl(robot) {
  if (!robot || !robot.joints) return;

  const jointNames = getSortedJointNames(robot);
  console.log('WebSocket Control - Available joints:', jointNames);
  console.log('WebSocket Control - All robot joints:', Object.keys(robot.joints));
  
  const wsStatusText = document.getElementById('wsStatus');
  const wsDot = document.getElementById('wsDot');

  // WebSocket 连接地址，默认连接本地 8765 端口
  ...
  function connect() {
    ...
    socket.onmessage = (event) => {
      // 检查是否启用了 WebSocket 控制
      const enableWS = document.getElementById('enableWS');
      if (enableWS && !enableWS.checked) {
        console.log('WebSocket control disabled by checkbox');
        return;
      }
      ...
      try {
        ...
        // 期望数据格式: [servo1_deg, servo2_deg, ..., servo6_deg]
        if (Array.isArray(data) && data.length >= jointNames.length) {
          console.log('Processing angles for joints:', jointNames);
          
          data.forEach((angleDeg, index) => {
            if (index < jointNames.length) {
              const jointName = jointNames[index];
              const joint = robot.joints[jointName];
              console.log(`Joint ${index + 1} (${jointName}): angle=${angleDeg}°`);
              
              if (joint) {
                let angleRad = MathUtils.degToRad(angleDeg);
                
                // 自动限位：如果超出范围，则强制限制在限位边缘，而不是直接跳过
                if (joint.jointType !== 'continuous' && joint.jointType !== 'fixed') {
                  const originalAngle = angleRad;
                  angleRad = Math.max(joint.limit.lower, Math.min(joint.limit.upper, angleRad));
                  if (originalAngle !== angleRad) {
                    console.log(`  Joint ${jointName} clamped: ${originalAngle}rad -> ${angleRad}rad`);
                  }
                }
                
                console.log(`  Setting ${jointName} to ${angleRad}rad (${MathUtils.radToDeg(angleRad)}°)`);
                joint.setJointValue(angleRad);
                
                // 如果连接了真实机器人，同步发送指令
                if (isConnectedToRealRobot) {
                  const servoId = index + 1;
                  const targetPosition = Math.round(2048 + (angleRad * 4096 / (2 * Math.PI)));
                  writeServoPosition(servoId, targetPosition).catch(err => {
                    console.error(`WS sync to servo ${servoId} failed:`, err);
                  });
                }
              } else {
                console.error(`  Joint ${jointName} not found in robot!`);
              }
            }
          });
          ...
          if (robot.updateMatrixWorld) {
            robot.updateMatrixWorld(true);
            console.log('Robot matrix updated');
          }
        } else {
          console.warn('Invalid data format. Expected array of', jointNames.length, 'angles');
        }
      } catch (err) {
    ...
    socket.onclose = () => {
      ...
      if (wsStatusText) wsStatusText.textContent = '断开重连中...';
