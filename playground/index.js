...
// 导入控制工具函数
import { setupKeyboardControls, setupControlPanel, setupAngleDirectControl, setupWebSocketControl } from './robotControls.js';
...
function init() {
  ...
  function loadModelFromHash() {
    ...
    manager.onLoad = () => {
      ...
      // Initialize direct angle controls
      setupAngleDirectControl(window.robot);

      // Initialize WebSocket control
      setupWebSocketControl(window.robot);
    };
  }
...
function logJointLimits(robot) {
  ...
  console.log("Robot servo limits:");
  Object.entries(robot.joints).forEach(([name, joint]) => {
    ...
    if (joint.jointType !== 'fixed' && joint.jointType !== 'continuous') {
      ...
      console.log(`  No limits (continuous servo)`);
    } else {
      console.log(`  No limits (fixed servo)`);
    }
  });
}
